...
// 2. In ESM mode, append `.mjs` to extensionless `@trezor/<pkg>/libESM/<sub>` imports
...
//   e.g. node post-build.mjs ./lib cjs
//        node post-build.mjs ./libESM esm
...
const libDir = mode === 'esm' ? 'libESM' : 'lib';
...
// In ESM mode, append `.mjs` to extensionless `@trezor/<pkg>/libESM/<sub>` imports.
// Resolve directory vs file by inspecting the source workspace package.
const TREZOR_LIBESM_RE = /(@trezor\/[a-zA-Z0-9-]+\/libESM\/[^'"`)\s]+)/g;

function appendMjsToTrezorLibESM(content) {
    return content.replace(TREZOR_LIBESM_RE, match => {
        if (/\.(mjs|cjs|js|json)$/.test(match)) return match;
        const m = match.match(/^@trezor\/([a-zA-Z0-9-]+)\/libESM\/(.+)$/);
        if (!m) return match;
        const [, pkg, sub] = m;
...
for (const file of allFiles) {
    ...
    if (mode === 'esm') {
        rewritten = appendMjsToTrezorLibESM(rewritten);
