...
import { getBatchingTransport } from './utils/getBatchingTransport';
import {
...
class SolanaWorker extends BaseWorker<SolanaAPI> {
    ...
    async tryConnect(url: string): Promise<SolanaAPI> {
        ...
        const throttledTransport = getThrottledTransport(transport, THROTTLE_OPTIONS);
        const batchedTransport = getBatchingTransport(
            throttledTransport,
            clusterUrl,
        ) as RpcTransportMainnet;
        const throttledRpc = createSolanaRpcFromTransport(batchedTransport);
