...
const config: webpack.Configuration = {
    ...
    output: {
        ...
        chunkFilename: a => {
            ...
            if (name && /^shared\//.test(name)) return '[name].js';
            if (name && /-worker$/.test(name)) return 'workers/[name].js';
            if (name && /-api-index-ts$/.test(name))
                return `coins/${name.replace(/-index-ts$/, '')}.js`;
